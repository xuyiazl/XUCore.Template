        ...
        public class Validator : CommandValidator<UserCreateCommand>
        {
            public Validator()
            {
                RuleFor(x => x.UserName).NotEmpty().MaximumLength(20).WithName("账号")
                    ...
                    {
                        var res = await Web.GetService<IUserAppService>().GetAnyAsync(AccountMode.UserName, account, 0, cancel);
                ...
                RuleFor(x => x.Mobile)
                    ...
                    .MustAsync(async (account, cancel) =>
                    {
                        var res = await Web.GetService<IUserAppService>().GetAnyAsync(AccountMode.Mobile, account, 0, cancel);
