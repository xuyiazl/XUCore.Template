...
$(function () {
    ...
    var columns = [
        ...
        {
            field: 'SchoolName', title: '学校名称', sortable: true, align: 'left'
        },
        {
            field: 'Specialty', title: '所学专业', sortable: false, align: 'center'
        },
        {
            field: 'Certificate', title: '所获证书', sortable: false, align: 'center'
        },
        {
            field: 'BeginDate', title: '入学日期', sortable: true, align: 'center', formatter: function (val, row) {
                var date = formatterTime(val, 'YYYY-MM');

                if (date.indexOf('1901') > -1 || date.indexOf('9999') > -1) return '至今';

                return date;
            }
        },
        {
            field: 'EndDate', title: '毕业日期', sortable: true, align: 'center', formatter: function (val, row) {
                var date = formatterTime(val, 'YYYY-MM');
    ...
    var contextMenuItem = function (row, $el) {
        ...
        switch (item) {
            case 'edit':
                window.parent.addTab({ id: 'resume-education-update-' + row.Id, href: '/admin/resume/educationupdate/' + row.Id, icons: 'fa fa-plus-circle m-r-xs', text: '修改数据' + row.Id });
                break;
            case 'publish':
