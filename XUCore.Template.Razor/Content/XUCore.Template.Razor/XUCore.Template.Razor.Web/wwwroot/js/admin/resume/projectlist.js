...
$(function () {
    ...
    var columns = [
        ...
        {
            field: 'ProjectName', title: '项目名称', sortable: true, align: 'left'
        },
        {
            field: 'BeginDate', title: '开始日期', sortable: true, align: 'center', formatter: function (val, row) {
        ...
        {
            field: 'EndDate', title: '结束日期', sortable: true, align: 'center', formatter: function (val, row) {
                var date = formatterTime(val, 'YYYY-MM');
    ...
    var contextMenuItem = function (row, $el) {
        ...
        switch (item) {
            case 'edit':
                window.parent.addTab({ id: 'resume-project-update-' + row.Id, href: '/admin/resume/projectupdate/' + row.Id, icons: 'fa fa-plus-circle m-r-xs', text: '修改数据' + row.Id });
                break;
            case 'publish':
