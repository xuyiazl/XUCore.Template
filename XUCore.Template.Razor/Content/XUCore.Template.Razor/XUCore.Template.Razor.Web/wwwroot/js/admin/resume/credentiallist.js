...
$(function () {
    ...
    var columns = [
        ...
        {
            field: 'Organization', title: '认证机构', sortable: true, align: 'left'
        },
        {
            field: 'Credential', title: '所获证书', sortable: false, align: 'center'
        },
        {
            field: 'AcquireDate', title: '获取日期', sortable: true, align: 'center', formatter: function (val, row) {
    ...
    var contextMenuItem = function (row, $el) {
        ...
        switch (item) {
            case 'edit':
                window.parent.addTab({ id: 'resume-credential-update-' + row.Id, href: '/admin/resume/credentialupdate/' + row.Id, icons: 'fa fa-plus-circle m-r-xs', text: '修改数据' + row.Id });
                break;
            case 'publish':
