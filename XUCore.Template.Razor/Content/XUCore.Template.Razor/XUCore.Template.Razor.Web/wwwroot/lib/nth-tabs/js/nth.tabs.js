...
(function ($) {
    $.fn.nthTabs = function (options) {
        ...
        var template = '<div class="page-tabs">\
				...
				<div class="dropdown roll-nav right-nav-list">\
					...
					<ul class="dropdown-menu">\
						<li><a href="#" class="tab-location"><i class="fa fa-map-marker m-r-xs"></i>定位当前选项卡</a></li>\
						<li><a href="#" class="tab-close-current"><i class="fa fa-minus-circle m-r-xs"></i>关闭当前选项卡</a></li>\
						<li role="separator" class="divider"></li>\
						<li><a href="#" class="tab-close-other"><i class="fa fa-minus-circle m-r-xs"></i>关闭其他选项卡</a></li>\
						<li><a href="#" class="tab-close-all"><i class="fa fa-minus-circle m-r-xs"></i>关闭全部选项卡</a></li>\
						<li role="separator" class="divider"></li>\
        ...
        var methods = {
            ...
            //事件监听
            listen: function () {
                event.onTabClose().onTabDbClose().onTabRollLeft().onTabRollRight().onTabList()
                    .onTabCloseOpt().onTabCloseAll().onTabCloseOther().onLocationTab().onTabRefresh();
            },
            //获取所有tab宽度
            ...
            addTab: function (options) {
                ...
                if (navTabA.length > 0) {
                    ...
                    allowClose ?
                        tab.push('<a href="#' + options.id + '" class="tab-db-close" data-toggle="tab">') :
                        tab.push('<a href="#' + options.id + '" data-toggle="tab">');
                    tab.push('<span>' + options.title + '</span>');
                    allowClose ? tab.push('<i class="fa fa-minus-square tab-close"></i>') : '';
                    tab.push('</a>');
                    tab.push('</li>');
                    ...
                    tabContent.push('<iframe src="' + options.href + '" scrolling="no" frameborder="0"></iframe>');
                    tabContent.push('</div>');
                    nthTabs.find(".tab-content").append(tabContent.join(''));

                    $(nthTabs.find("[href='" + tabId + "']")).contextmenu(function (e) {
                        e.preventDefault();
                        methods.refreshTab(tabId);
                    });
                }
                return methods;
            },
            ...
            refreshTab: function (tabId) {
                ...
                var _iframe1 = $('iframe', tabId).get(0);
        ...
        var event = {
            ...
            //双击关闭选项卡
            onTabDbClose: function () {
                nthTabs.on("dblclick", '.tab-db-close', function () {
                    var tabId = $(this).parent().attr('href');
                    methods.delTab(tabId);
                });
                return event;
